...
static pngquant_error read_image(liq_attr *options, struct rwpng_data * in_buffer, int using_stdin, png24_image *input_image_p, liq_image **liq_image_p, bool keep_input_pixels);
static pngquant_error write_image(png8_image *output_image, png24_image *output_image24, struct rwpng_data * out_buffer, struct pngquant_options *options);
...
int pngquant_file(struct rwpng_data *in_buffer, struct rwpng_data * out_buffer, struct pngquant_options *options);
...
int pngquant_file(struct rwpng_data *in_buffer, struct rwpng_data * out_buffer, struct pngquant_options *options)
{
    ...
    if (!retval) {
        retval = read_image(options->liq, in_buffer, options->using_stdin, &input_image_rwpng, &input_image, keep_input_pixels);
    }
    ...
    if (!retval) {
        ...
        retval = write_image(&output_image, NULL, out_buffer, options);
...
static pngquant_error write_image(png8_image *output_image, png24_image *output_image24, struct rwpng_data * out_buffer, struct pngquant_options *options)
{
    ...
    if (options->using_stdin) {
        ...
        if (out_buffer == NULL) {
            fprintf(stderr, "error: out buffer not initial!\n");
...
static pngquant_error read_image(liq_attr *options, struct rwpng_data * in_buffer, int using_stdin, png24_image *input_image_p, liq_image **liq_image_p, bool keep_input_pixels)
{
    ...
    #pragma omp critical (libpng)
    {
        retval = rwpng_read_image24(in_buffer, input_image_p);
    }
...
int main(int argc, char *argv[]) {
    struct rwpng_data * input_buffer;
    ...
    input_buffer = (struct rwpng_data *) malloc(sizeof(struct rwpng_data));

    input_buffer->length = filesize;
    input_buffer->png_data = image_buffer;
    ...
    fwrite(out_buffer->png_data, out_buffer->length, 1, fpin);
    fclose(fpin);
    free(out_buffer);
    free(input_buffer);
}
